    ...
    def do(self, wrld):
        ...
        if self.state == 'SAFE':
            ...
            if len(path) > 1:
                ...
                self.yeet(dx, dy)
                pass
            elif (self.x + dx, self.y + dy) is ex:
                print("ASTAR")
                self.yeet(dx, dy)
                pass
                return
                ...
                good_moves = get_safe_moves(self, (self.x, self.y), wrld, monsters, bombs)
                if len(good_moves) is 0:
                    print("NO INTERSECTION")

                    valid = valid_moves(wrld, (self.x, self.y))



                    print("VALID::: ", valid)
                    for m in monsters:
                        print("MONSTER:::", monster_can_be_in(m))
                    for m in monsters:
                        valid = list(set(valid).difference(set(monster_can_be_in(m))))


                    for move in valid:
                        good_moves.append((move[0] - self.x, move[1]-self.y))



                    # for move in good_moves:
                        # xDist, yDist, desiredMove, sdf, danger = bombDistance()
...
def monster_can_be_in(monster):
    crds = [monster]

    crds.append((monster[0]-1, monster[1]-1))
    crds.append((monster[0], monster[1]-1))
    crds.append((monster[0]+1, monster[1]-1))
    crds.append((monster[0], monster[1]+1))
    crds.append((monster[0]-1, monster[1]))
    crds.append((monster[0]-1, monster[1]+1))
    crds.append((monster[0]+1, monster[1]+1))
    crds.append((monster[0]+1, monster[1]))
...
def best_move_away(list_moves, bombs, monsters):
    best = -float('inf')
    desired = None
    for move in list_moves:
        ...
        for m in monsters:
            for b in bombs:
                score += 1 / abs(move[0] - b[0]) + abs(move[1] - b[1])
            score += 1 / abs(move[0] - m[0]) + abs(move[1] - m[1])
